...
def drop_cjk_comments(text: str) -> str:
    """refs.bib carries % comments that are internal working notes written in
    Chinese (for instance, that a subagent verified each bibliography entry against
    the original page). strip_fields passes them through unchanged, so they would
    ship inside the arXiv package and stay there permanently. Drop every comment
    line containing CJK characters; the BibTeX entries themselves are untouched.
    """
    kept = [l for l in text.splitlines()
            if not (l.lstrip().startswith("%") and has_cjk(l))]
    return chr(10).join(kept) + chr(10)


def write_refs_public() -> pathlib.Path:
    out = HERE / "refs_public.bib"
    out.write_text(drop_cjk_comments(strip_fields(BIB.read_text(encoding="utf-8"), set(DROP_FIELDS))),
                   encoding="utf-8")
...
def fix_sin_frac(tex: str) -> str:
    """pandoc turns the Typst expression `sin theta / 2` (which Typst reads as
    sin(theta)/2) into a LaTeX sine-of-a-half-angle fraction, i.e. sin(theta/2).
    The two differ at theta = pi/4: the correct value is 0.353553, the mangled one
    is 0.382683. Section 3.5 prints that formula next to its own value 0.353553, so
    the mangled form contradicts the sentence that follows it. Only this shape is
    restored, and the result is checked fail-closed.
    """
    pat = re.compile(re.escape(B) + "sin" + re.escape(B) + "frac[{]([^{}]*)[}][{]([^{}]*)[}]")
    out = pat.sub(lambda m: B + "frac{" + B + "sin " + m.group(1) + "}{" + m.group(2) + "}", tex)
    if pat.search(out):
        raise SystemExit("fix_sin_frac: un-restored sin-frac remains")
    return out
...
def to_latex(typst_src: str) -> str:
    ...
    return fix_sin_frac(fix_subscript_parens(proc.stdout))
...
def build_en(keys: set) -> None:
    ...
    head = chr(10).join([
        "% Generated by make_latex.py from paper_en.typ -- do not edit; edit the Typst source.",
        ...
        "  pdfauthor={Xin Yang, Poyuan Chung, Yuan-Liang Zhong},"
...
def build_zh(keys: set) -> None:
    ...
    head = chr(10).join([
        "% Generated by make_latex.py from paper.typ -- do not edit; edit the Typst source.",
