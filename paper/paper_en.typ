...
#show: arxiv-style.with(
  ...
  abstract: [
    ...
    layer of a classifier, not an attempt to demonstrate quantum advantage. The classifier
    is trained on a controlled synthetic $8$-class task rather than on a benchmark
    dataset, so that what is measured is the behaviour of the classifier itself. Building
...
(Section 4.3), they immediately fix the expectation for all four ablation arms---
...
protocol, described in Section 4.4 and Section 6.2. There, $T_1$ amplitude damping
...
The classification task of this study is a *controlled synthetic problem*, not a
benchmark. That is deliberate: what is measured here is the quality of a
classifier---its accuracy and its calibration---as the quantum layer above it
changes. Running that measurement on a benchmark dataset would confound it with the
difficulty of one particular dataset, which is not what this study is about.

The inputs are generated by NumPy and nothing else, from the fixed seed $12345$.
Each sample is a $256$-dimensional standard-normal vector. There are $8$ classes and
each class has a prototype vector, drawn once, which is added to the first $12$
components of every sample of that class; only those $12$ components carry signal.
Principal component analysis, the centring mean and the min--max rescaling are all
fitted on the *training* split alone, so nothing from the test split enters the
preprocessing. The split is $80$ training and $300$ test samples, and it is the same
for every configuration.

The classical front end is therefore a *PCA feature vector of a synthetic input*,
not a learned embedding of real data.

*Nested input, and a confound stated up front.* The $n$ qubits of a configuration
receive the first $n$ principal components. This is deliberate---it makes "more
qubits" mean "more of the same task"---but it also means that $n$ is not a single
variable: raising $n$ raises the trainable capacity *and* the number of input
features at the same time. Section 6.1 reports what follows from that, and
Section 7.3 returns to it as a limitation.

*Training.* Adam, learning rate $0.05$; $1200$ steps for the capacity scan and $800$
for the ablation arms; $5$ seeds per configuration, listed in Section 5.3.
== System architecture
...
#figure(
  ...
  caption: [
    System architecture. The classical front end (principal component analysis, PCA, applied to the input feature vector)
...
+ This work *does not claim* that the quantum layer retains the semantic information of
  its input. The front end is a $256$-dimensional synthetic input that is reduced in
...
cross-validation of Section 4.5, the capacity-scan and calibration-ablation
