\begin{table}[t]
\centering
\caption{Test accuracy as a function of qubit count at fixed circuit
depth (mean over five seeds; the smaller grey value is the standard
deviation). \(n = 3\) is structurally low: eight classes require at
least a three-qubit projective readout, so that point is exactly
saturated. The claim is therefore stated as ``from \(n = 4\)
onward''.}
\resizebox{\ifdim\width>\linewidth \linewidth\else\width\fi}{!}{%
\begin{tabular}{@{}llll@{}}
\toprule
\textbf{Qubits} \(n\) & \textbf{\(L\) = 4} & \textbf{\(L\) = 6} &
\textbf{\(L\) = 8} \\
\midrule
...
\begin{table}[t]
\centering
\caption{The complete \((n,L)\) grid: mean test accuracy over five
seeds, with the standard deviation in parentheses. A dash means the cell
does not have all five seeds; incomplete cells are excluded rather than
pooled, so that a single-seed standard deviation of \(0\) is never mixed
with a five-seed one.}
\resizebox{\ifdim\width>\linewidth \linewidth\else\width\fi}{!}{%
